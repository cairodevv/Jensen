...
std::pair<int, int> guessNum(int number, int changeDifference) {
    int tempChangeDifference = {changeDifference};
    bool finished = {false};
    int guess = {rand() % 101};
    int loss = {abs(number - guess)};
...
int main() {
    ...
    std::cin >> number;
    std::cin >> diff;
    std::pair<int, int> guess = guessNum(number, diff);
    std::cout << "\n";
    std::cout << "Guess: " << guess.first << "\n";
    std::cout << "Loss: " << guess.second << "\n";
    return 0;
}